...
def vlc_import():
    ...
    for index, path in enumerate(possible_vlc_paths):
            ...
            if "vlc.exe" in os.listdir(path):
                ...
                try:
                    ...
                    if not os.path.isdir("../temp"):
                        os.mkdir("../temp")
                    with open("../temp/hasvlc.tmp", "w") as _:
                        pass
                    break
                except OSError:
                    ...
                    sys.exit(
                        "VLC module was not found. Please install all required modules using 'py -m pip install -r requirements.txt'"
                    )
        ...
        print(
            'If you already have VLC Media Player installed, please enter the path of its install directory (containing "vlc.exe" file) in the config file, under the heading "VLC PATH"\n\n'
        )

    if VLC_ARCH:
        if VLC_ARCH != PY_ARCH:
            sys.exit(
                "ERROR: Detected incompatible architecture of VLC Media Player. Please uninstall your current installation of VLC Media Player app and download the {0} bit version instead.\Visit https://www.videolan.org/ to download: ".format(
                    PY_ARCH
                )
            )
