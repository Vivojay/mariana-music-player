...
if not os.path.isdir(
    os.path.join(os.environ["localappdata"], f"Mariana Music Player v{MARIANA_PLAYER_VERSION_FRIENDLY_NAME}")
):
    os.mkdir(os.path.join(os.environ["localappdata"], f"Mariana Music Player v{MARIANA_PLAYER_VERSION_FRIENDLY_NAME}"))

RUN_ONCE = os.path.isfile(
    os.path.join(
        os.environ["localappdata"], f"Mariana Music Player v{MARIANA_PLAYER_VERSION_FRIENDLY_NAME}\\setup.pypaths"
    )
)

GIT_INSTALLED = not subprocess.call(
    ["git", "--version"], shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
)
...
if NO_CONF:
    _Print("NO CONFIRMATION MODE ENABLED...")
...
if not (SILENT or NO_CONF):
    ...
    while True:
        if perm.lower().strip() == "y":
            ...
            sys.exit(
                "Shutting down program (Try '--no-confirm' flag for automatic setup w/o confirmations).\nSetup aborted by user..."
            )
...
if not RUN_ONCE:
    ...
    if os.path.exists(default_python_path):
        ...
        default_python_ver_string = (
            subprocess.run(f"{default_python_path} -c {python_ver_command}", capture_output=1).stdout.decode().strip()
        )
        ...
        if any(supported_py_vers_installed):
            ...
            found_python_ver_string = (
                subprocess.run(f"{supported_py_vers_installed[0]} -c {python_ver_command}", capture_output=1)
                .stdout.decode()
                .strip()
            )
            ...
            sys.exit(
                "No python version below 3.10 found, please install a compatible python version and rerun this setup.\nAborting this auto setup..."
            )
    ...
    try:
        if not (SILENT or NO_CONF):
            PROG_DIR_PATH = input(
                "Please make a new empty directory for this program to install into and\nenter its path here (If directory is non-empty, setup will abort): "
            )
    except KeyboardInterrupt:
        sys.exit("\nUser successfully aborted this setup...")
    ...
    if os.path.isdir(PROG_DIR_PATH):
        ...
        while True:
            if CREATE_NEW_DIR_PERM.lower().strip() == "y":
                ...
                print(
                    "You have aborted the installation temporarily. Create dir yourself and rerun this setup script..."
                )
                sys.exit(0)

            try:
                CREATE_NEW_DIR_PERM = input(
                    f"[INVALID RESPONSE, RETRY] Do you want to create directory at {PROG_DIR_PATH}? [y/n]: "
                )
            except KeyboardInterrupt:
                sys.exit("\nUser successfully aborted this setup...")
    ...
    try:
        if not (SILENT or NO_CONF):
            _ = input(
                f"Press enter to confirm this one-time setup with python version ({FINAL_VER[0]}), (CTRL+C to abort): "
            )
    except KeyboardInterrupt:
        sys.exit("\nUser successfully aborted this setup...")
    ...
    if not SETUP_INFO["SETUP_COMPLETE"]:
        ...
        if sys.executable in SETUP_INFO["INSTALLED_PYS"]:
            ...
            if sys.executable != PYTHON_EXECUTABLE:
                ...
                with open(
                    os.path.join(os.environ["localappdata"], "Mariana Music Player v0.4.2\\setup.pypaths"), "w"
                ) as f:
                ...
                if not SILENT:
                    ...
                    print(
                        f'Run either one using ".\\runner.bat" (Recommended) or ".\\runner.ps1"\nto start the music player in one go, whenever you like!\n'
                    )
                    ...
                    print(
                        "  First, open powershell as admin and run: 'Set-ExecutionPolicy Unrestricted' (without quotes)"
                    )
        ...
        print(
            "Setup is complete, refer to the runner.bat and runner.ps1 files created in the directory where you downloaded the app..."
        )
