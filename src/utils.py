...
APP_BOOT_START_TIME = time.time()
print("Loaded 1/25", end="\r")

import os

print("Loaded 2/25", end="\r")
...
print("Loaded 4/25", end="\r")
import sys

print("Loaded 5/25", end="\r")
import urllib

print("Loaded 6/25", end="\r")
import pygame

print("Loaded 7/25", end="\r")
import numpy as np

print("Loaded 8/25", end="\r")
import random as rand

print("Loaded 9/25", end="\r")
import importlib

print("Loaded 10/25", end="\r")
import colored

print("Loaded 11/25", end="\r")

# import concurrent.futures;                          print("Loaded 11/25", end='\r')

from tabulate import tabulate as tbl

print("Loaded 12/25", end="\r")
from ruamel.yaml import YAML

print("Loaded 13/25", end="\r")
from collections.abc import Iterable

print("Loaded 14/25", end="\r")
from logger import SAY

print("Loaded 15/25", end="\r")
from multiprocessing import Process

print("Loaded 16/25", end="\r")
...
try:
    ...
    if not FIRST_BOOT:
        sys.exit(
            "[INFO] Could not find lib.lib file, '\
...
def first_startup_greet(is_first_boot):
    if is_first_boot:
        try:
            ...
            sys.exit(
                "[ERROR] Critical guide setup-file missing, please consider reinstalling this file or the entire program\nAborting Mariana Player. . ."
            )
...
try:
    with open("../user/user_data.yml", encoding="utf-8") as u_data_file:
        ...
        if list(USER_DATA.keys()) == ["default_user_data"]:
            if (
                not FIRST_BOOT and not ISDEV
            ):  # TODO - remove temp ISDEV flag after user login and register feature is fully functional
                SAY(
                    visible=visible,
                    display_message="",
...
def reload_sounds():
    ...
    if not no_lib_found:
        ...
        _sound_files = [
            [list(audio_file_gen(paths[j], supported_file_exts[i])) for i in range(len(supported_file_exts))]
...
try:
    _ = sp.run("ffmpeg", stdout=sp.DEVNULL, stdin=sp.PIPE, stderr=sp.DEVNULL)
except FileNotFoundError:
    FATAL_ERROR_INFO = "ffmpeg not recognised globally, download it and add to path (system environment)"
...
def save_user_data():
    ...
    total_plays = [
        j
        for i, j in USER_DATA["default_user_data"]["stats"]["play_count"].items()
...
def exitplayer(sys_exit=False):
    ...
    SAY(visible=visible, display_message="", log_message=f"Time spent using app = {time_spent_on_app}", log_priority=3)
...
def vol_trans_process_spawn():
    vol_trans_process = Process(
        target=voltransition, args=({"initial": cached_volume, "final": 0, "disablecaching": True})
    )
        ...
        if len(commandslist) == 2:
            ...
            if songindex.isnumeric():
                if int(songindex) in range(len(_sound_files) + 1):
                    currentsong_length = None
                    play_local_default_player(_sound_files[int(songindex) - 1], _songindex=songindex)
                else:
...
def timeinput_to_timeobj(rawtime):
    try:
        if ":" in rawtime.strip():
            ...
            timeobj = [
                value * 60 ** (len(processed_rawtime) - _index - 1) for _index, value in enumerate(processed_rawtime)
            ]
            ...
            formattedtime = " ".join(
                [
                    ...
                    for i in list(zip(processed_rawtime, ["h", "m", "s"][3 - len(processed_rawtime) :]))
                ]
            )
    ...
    if timeval:
        if current_media_player:
            ...
            try:
                ...
                SAY(
                    visible=visible,
                    display_message="Error: Can't seek in this song",
                    log_message=f"Unsupported codec for seeking song: {currentsong}",
                    log_priority=2,
                )
                return None
...
def setmastervolume(value=None):
    ...
    if comtypes_load_error:
        ...
        if value in range(101):
            ...
            SAY(
                visible=visible,
                display_message="ERROR: Could not set master volume",
                log_message="Could not set master volume",
                log_priority=2,
            )
...
def play_vas_media(media_url, single_video=None, media_name=None, print_now_playing=True, media_type="video"):
    ...
    if media_type == "video":
        ...
        vas.set_media(
            _type=f"radio/{media_name}"
        )  # No need for an explicit `audurl` here... (as per definition of vas.set_media)
        ...
        SAY(
            visible=visible,
            display_message="Invalid media type provided",
            log_message="Invalid media type provided",
            log_priority=2,
        )
    ...
    while not vas.vlc_media_player.get_media_player().is_playing():
        pass
    while not vas.vlc_media_player.get_media_player().get_length():
        pass
    ...
    if currentsong_length == -1:
        SAY(visible=visible, log_message="Cannot get length for vas media", display_message="", log_priority=3)
    isplaying = True
...
def choose_media_url(media_url_choices: list, yt: bool = True):
    ...
    if yt:
        if len(media_url_choices) == 1:
            ...
            chosen_index = input(
                f"{colored.fg('deep_pink_4c')}Choose video number between 1 and {len(media_url_choices)}"
                f" (leave blank to skip): {colored.fg('navajo_white_1')}"
            ).strip()
            ...
            if chosen_index:
                ...
                if chosen_index in range(1, len(media_url_choices) + 1):
                    ...
                    play_vas_media(media_name=media_name, media_url=media_url, single_video=False)
                else:
                    print("ERROR: Invalid choice, choose again: ", end="\r")
...
def process(command):
    ...
    if commandslist != []:  # Atleast 1 word
        ...
        if commandslist in [["exit"], ["quit"], ["e"]]:
            perm = input(
                colored.fg("light_red") + "Do you want to exit? [Y]es, [N]o (default = N): " + colored.fg("magenta_3c")
            )
            print(colored.attr("reset"), end="")
        ...
        if commandslist[0] in ["list", "ls"]:
            ...
            if len(commandslist) == 1:
                ...
                if indices:
                    ...
                    print(
                        f'Regex search is still in progress... The developer @{ABOUT["about"]["author"]} will add this feature shortly...'
                    )
                    # regex_pattern
            ...
            SAY(
                ...
                log_message=f"Command assumed to be misspelled: {currentsong}",
                log_priority=3,
            )
                ...
                if len(commandslist) == 2:
                    ...
                    if not time_validity:  # Raw time is valid
                        ...
                        # When using relative seek
                            ...
                            SAY(
                                visible=visible,
                                display_message="Error: Seek value too large for this song",
            ...
            if currentsong:
                ...
                if commandslist[0].endswith("*"):
                    ...
                    print(
                        f"progress: {colored.fg('deep_pink_1a')}{round(cur_prog)}/{round(cur_len)}"
                    ...
                    print(
                        f"{colored.fg('deep_pink_1a')}{round(cur_prog)}/{round(cur_len)}"
            ...
            if currentsong_length:
                try:
                    ...
                    SAY(
                        visible=visible,
                        display_message="Error: Can't reset this song",
            ...
            try:
                ...
                if all(
                    [not i.replace(" ", "").isnumeric() for i in command.split(".")]
                ):  # Identifying a file-existence-check command
        ...
        # TODO: Refactor to replace two `err` funcs with one
            ...
            print(
                f'Weblinks feature is still in progress... The developer @{ABOUT["about"]["author"]} will add this feature shortly...'
            )
            ...
            if current_media_type == 0:  # If YT vid is playing...
                ...
                print(
                    f'Reddit session syncing... The developer @{ABOUT["about"]["author"]} will add this feature shortly...'
                )
            ...
            try:
                if len(commandslist) == 2 and commandslist[1].isnumeric():
                    if "." in commandslist[1]:
                        SAY(
                            visible=visible,
                            display_message="Volume must not have decimal point precision",
                            log_message="Volume set to decimal percentage",
                            log_priority=2,
                        )
                    else:
                        ...
                        if volper in range(101):
                            ...
                            SAY(
                                visible=visible,
                                display_message="Volume percentage is out of range, it must be between 0 and 100",
                                log_message="Volume percentage out of range",
                                log_priority=2,
                            )
            ...
            print(
                "Sorry, system volume commands have been (temporarily) disabled due to some internal issue (Issue #244 comtypes)"
            )
        ...
        #      /ys "The Weeknd Blinding Lights" 4
            ...
            try:
                ...
                if rescount == "":
                    try:
                        ...
                        err("Could not load video... (Maybe check your VPN?)", "Video Load Error", say=False)
                    ...
                    if int(rescount) in range(2, max_yt_search_results_threshold + 1):
                        ytv_choices = YT_query.search_youtube(search=qr_val, rescount=int(rescount))
                    else:
                        SAY(
                            visible=visible,
                ...
                raise
                err(
                    'Invalid YouTube search, type: [/youtube-search | /ys] "<search terms>" [<result_count>]', say=False
                )
            ...
            if len(commandslist) in [1, 2]:
                ...
                if radio_media:
                    ...
                    print(
                        tbl(
                            [
                                (f"{colored.fg('light_red')}/wra {i+1}{colored.attr('reset')}", j)
                                for i, j in enumerate(r_stations)
                            ],
                            tablefmt="plain",
                        )
                    )
            ...
            if r_seshs:
                ...
                if len(commandslist) == 1:
                    r_seshs_table = tbl(
                        r_seshs_data_processed, tablefmt="simple", headers=["#", "RPAN Session"] + [*rs_params[1:]]
                    )
                    print(r_seshs_table)
                    print()
                    sesh_index = input(
                        f"{colored.fg('light_slate_blue')}Enter RPAN session number to tune into: {colored.fg('navajo_white_1')}"
                    )
                    print(colored.attr("reset"), end="")
                elif len(commandslist) == 2:
