...
test('packaged Electron app launches its bundled CLI backend', async () => {
  ...
  try {
    ...
    await writeCommand(page, 'theme gruvbox')
    await expect(page.getByLabel('Terminal theme')).toHaveValue('gruvbox')
    await page.getByLabel('Search terminal').fill('Sleep timer')
    await page.getByLabel('Find next').click()
    await expect(page.getByLabel('Search terminal')).toHaveValue('Sleep timer')
    await writeCommand(page, 'help playback')
    await expect(page.locator('.terminal-pane:not([hidden])').getByLabel('Terminal output')).toContainText('Playback')
