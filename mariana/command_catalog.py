...
COMMAND_CATALOG = (
    ...
    _spec("library.list", "ls", CommandCategory.LIBRARY, "List or filter indexed media with a regular expression", forms=_forms(arguments=("count, indexes, range, or regex",), flags=("o", "desc"))),
    _spec("library.recents", "recents", CommandCategory.LIBRARY, "List or filter recently played media with a regular expression", forms=_forms(arguments=("count, indexes, range, or regex",), flags=("o", "desc"))),
    ...
    _spec("search.find", "find", CommandCategory.ONLINE, "Search the library or an explicit collection", forms=_forms(arguments=("free-text or regex",), flags=("--in", "--regex", "--re"))),
    _spec("search.raw", "rfind", CommandCategory.ONLINE, "Search a collection literally", forms=_forms(arguments=("free-text or regex",), flags=("--in", "--regex", "--re"))),
    _spec("search.loose", "lfind", CommandCategory.ONLINE, "Search a collection loosely", forms=_forms(arguments=("free-text or regex",), flags=("--in", "--regex", "--re"))),
    ...
    _spec(
        ...
        "Browse the official public-domain audiobook catalog",
        forms=_forms("status", "help"),
    ),
    _spec("librivox.search", "librivox search", CommandCategory.ONLINE, "Search LibriVox audiobook titles", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("title",), flags=("--limit", "--offset"))),
    _spec("librivox.author", "librivox author", CommandCategory.ONLINE, "Find LibriVox audiobooks by author surname", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("surname",), flags=("--limit", "--offset"))),
    _spec("librivox.genre", "librivox genre", CommandCategory.ONLINE, "Find LibriVox audiobooks by genre", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("genre",), flags=("--limit", "--offset"))),
    _spec("librivox.recent", "librivox recent", CommandCategory.ONLINE, "List recently added LibriVox audiobooks", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("days",), flags=("--limit", "--offset"))),
    _spec("librivox.show", "librivox show", CommandCategory.ONLINE, "Inspect full LibriVox project metadata", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id",))),
    _spec("librivox.chapters", "librivox chapters", CommandCategory.ONLINE, "List audiobook sections and volunteer readers", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id",))),
    _spec("librivox.play", "librivox play", CommandCategory.ONLINE, "Play a LibriVox chapter with persistent book navigation", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id", "chapter"))),
    _spec("librivox.current", "librivox current", CommandCategory.PLAYBACK, "Show the current audiobook and chapter position", availability=("current-media",)),
    _spec("librivox.goto", "librivox goto", CommandCategory.PLAYBACK, "Play a numbered chapter in the current LibriVox audiobook", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("chapter",)), availability=("current-media",)),
    _spec("librivox.next", "librivox next", CommandCategory.PLAYBACK, "Move forward within the current LibriVox audiobook", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("count",)), availability=("current-media",)),
    _spec("librivox.previous", "librivox previous", CommandCategory.PLAYBACK, "Move backward within the current LibriVox audiobook", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("count",)), availability=("current-media",)),
    _spec("librivox.first", "librivox first", CommandCategory.PLAYBACK, "Play the first chapter in the current LibriVox audiobook", risk=CommandRisk.STATE_CHANGING, availability=("current-media",)),
    _spec("librivox.last", "librivox last", CommandCategory.PLAYBACK, "Play the final chapter in the current LibriVox audiobook", risk=CommandRisk.STATE_CHANGING, availability=("current-media",)),
    _spec("librivox.restart", "librivox restart", CommandCategory.PLAYBACK, "Restart the current LibriVox chapter", risk=CommandRisk.STATE_CHANGING, availability=("current-media",)),
    _spec("librivox.resume", "librivox resume", CommandCategory.PLAYBACK, "Resume the selected persistent LibriVox chapter", risk=CommandRisk.STATE_CHANGING, availability=("current-media",)),
    _spec("librivox.queue", "librivox queue", CommandCategory.QUEUE, "Queue one section or a complete LibriVox audiobook", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id", "chapter-or-all"))),
    _spec("librivox.download", "librivox download", CommandCategory.DOWNLOADS, "Confirm and download LibriVox audiobook sections", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id", "chapter-or-all"), flags=("--format", "--to", "--yes"))),
    _spec("librivox.rss", "librivox rss", CommandCategory.ONLINE, "Open an audiobook's official LibriVox RSS feed", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id",))),
    _spec("librivox.open", "librivox open", CommandCategory.ONLINE, "Open an official catalog, text, archive, ZIP, or RSS link", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(arguments=("result-or-id", "link-kind"))),
    _spec(
        "reddit.retired",
        "/rs",
        CommandCategory.ONLINE,
    ...
    _spec("session", "session", CommandCategory.PLAYBACK, "Inspect local session recording/replay status", forms=_forms("status", "list")),
    _spec("session.record", "session record", CommandCategory.PLAYBACK, "Record structured listening intent locally", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("recipe-name",))),
    _spec("session.stop", "session stop", CommandCategory.PLAYBACK, "Seal a recording or stop active recipe replay", risk=CommandRisk.STATE_CHANGING),
    ...
    _spec("banner", "banner", CommandCategory.SETTINGS, "Inspect occasion-aware startup greetings", forms=_forms("status", "show", "help", "preview")),
    _spec("banner.occasions", "banner occasions", CommandCategory.SETTINGS, "Enable or disable occasion greetings", risk=CommandRisk.STATE_CHANGING, forms=_forms("on", "off")),
    _spec("banner.country", "banner country", CommandCategory.SETTINGS, "Use the operating-system region or an explicit ISO country", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("auto-or-ISO-country",))),
    _spec("banner.country.detect", "banner country detect", CommandCategory.SETTINGS, "Confirm a country-only network lookup", risk=CommandRisk.EXTERNAL_ACTION, forms=_forms(flags=("--yes",))),
    _spec("banner.subdivision", "banner subdivision", CommandCategory.SETTINGS, "Select a regional holiday calendar", risk=CommandRisk.STATE_CHANGING, forms=_forms(arguments=("subdivision-code-or-none",))),
