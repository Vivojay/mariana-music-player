...
| Command catalog and autocomplete risk metadata | High-risk compound forms are separately classified: playlist delete/clear and missing-library cleanup are destructive; lyrics edit, library scan, and queue clear are state-changing; playlist export is an external action. Read-only parents such as `lyrics` and `library` remain read-only. Aliases retain their canonical risk. Serialized metadata contains no handler, confirmation callback, insertion text, path, credential, or private identifier, and autocomplete remains display-only. | **Safe (fixed after this audit)** | `mariana/command_catalog.py`; `tests/test_command_catalog.py`; `desktop/commandCatalog.test.ts`; non-executing selection coverage in `desktop/App.test.tsx`. |
...
The final batch slice adds explicit safe-catalog entries for guarded compound
commands. Risk labels are informational projection data only: autocomplete
cannot execute, confirm, or write a reconstructed command to the terminal.
