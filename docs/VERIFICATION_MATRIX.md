...
The source capability rows below also include deterministic post-release batch
evidence; they do not replace pending packaged or native acceptance.
...
| Mini-player | Single-instance lazy window; close/Escape-to-hide; cached validated snapshot; dedicated least-privilege preload; safe identity/source/queue/progress states; Play/Pause/Previous/Next identity-bound controls; chapter/current-segment and source-timeline preferred-region display; live/unknown suppression; read-only/no-seek regression coverage | Native visual, eligible-media controls, long-title, and packaged lifecycle smoke remain pending | Deterministic desktop and development E2E passed; native/package acceptance pending |
| Desktop mouse/tap seek | Pure pointer-to-source-time projection; finite/seekable/playable eligibility; typed preload/IPC/backend request; current-media binding; invalid/stale request refusal; preferred-region clamping; pending/error reconciliation from authoritative projection; no PTY injection | Real-media click/tap, paused-state, auto-advance, and packaged acceptance remain pending | Deterministic source tests passed; native/package acceptance pending |
