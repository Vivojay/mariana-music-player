...
| Safe autocomplete/dropdown | Immutable catalog validation, typed runtime boundary, pure bounded projection, prefix/alias ordering, stale-response rejection, combobox/listbox semantics, keyboard/pointer selection, and assertions that acceptance never writes to the PTY or executes a command | Development Electron E2E covers the real catalog and non-executing keyboard/pointer interaction; packaged screen-reader acceptance remains pending | Safe command explorer passed; terminal-line completion intentionally not implemented |
