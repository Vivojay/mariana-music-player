...
Latest dated release-candidate run: [2026-07-17](verification/2026-07-17.md).
Latest narrow source/native smoke: [2026-07-20](verification/2026-07-20.md).
The source capability rows below include deterministic post-release batch
evidence; neither report replaces pending packaged or native acceptance.
...
| Destructive command hardening | Bound playlist ID/name/revision before delete/clear confirmation; transactional stale-target refusal; atomic no-clobber lyrics sidecar creation; explicit compound-command risk metadata; alias and competing-creator regressions | Native confirmation wording and external-editor behavior | Deterministic source gates passed; native UX smoke pending |
| Playback projection contract | Exact schema-7 backend serialization; Electron runtime allowlisting; bounded types and cross-field invariants; unknown-field dropping; private/malformed projection rejection; duplicate/older event rejection; main-window and Mini-player revalidation | Development Electron E2E exercises backend readiness and both status consumers; packaged mixed-version/protocol diagnostics remain pending | Deterministic source gates passed; packaged compatibility smoke pending |
| Mini-player | Single-instance lazy window; close/Escape-to-hide; cached validated snapshot; dedicated least-privilege preload; safe identity/source/queue/progress and long-identity retention; Play/Pause/Previous/Next identity-bound controls; chapter/current-segment and source-timeline preferred-region display; live/unknown suppression; read-only/no-seek regression coverage | Native visual truncation/scaling, eligible real-media controls, auto-advance, and packaged lifecycle smoke remain pending | Deterministic desktop and existing development E2E passed; native/package acceptance pending |
| Desktop mouse/tap seek | Pure pointer-to-source-time projection; finite/seekable/playable eligibility; typed preload/IPC/backend request; current-media binding; invalid/stale request refusal; paused-state preservation; preferred-region clamping; pending/error reconciliation from authoritative projection; no PTY injection | Physical real-media click/tap, paused UI, auto-advance, and packaged acceptance remain pending | Deterministic source tests passed; native/package acceptance pending |
| Electron/PTTY | React/Vitest coverage plus four development Electron scenarios for PTY behavior, autocomplete safety, close-to-tray, and Mini-player lifecycle | Windows packaged scenarios and native PTY evidence exist; signed multi-OS package remains pending | Source-native and Windows unpacked passed; signing pending |
| Safe autocomplete/dropdown | Immutable catalog validation, explicit risk-labelled compound commands, typed runtime boundary, pure bounded projection, prefix/alias ordering, stale-response rejection, combobox/listbox semantics, keyboard/pointer selection, and assertions that acceptance never writes to the PTY or executes/confirms a command | Development Electron E2E covers the real catalog and non-executing keyboard/pointer interaction; packaged screen-reader acceptance remains pending | Safe command explorer passed; terminal-line completion intentionally not implemented |
...
| `lyrics`, `lyr`, `lyrics edit`, `lyr edit` | Modernized | Open identification/LRCLIB and durable adjacent `.lrc` editing with bound no-clobber creation |
...
| `rename short` | Native | Transactional indexed-file rename with collision, rollback, source-ID de-duplication, confidence reporting, and placeholder-only refusal |
